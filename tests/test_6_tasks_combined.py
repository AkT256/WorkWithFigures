...
def demo():
    ...
    # П.6.1 — 10 трапеций, фильтр по площади <0.3 (останется 6)
    base = [(0, 0), (1, 0), (0.9, 0.2), (0.1, 0.2)]
    # центрируем для корректного масштабирования
    cx = sum(x for x, _ in base) / len(base)
    cy = sum(y for _, y in base) / len(base)
    centered = [(x - cx, y - cy) for x, y in base]

    shapes1 = [
        tr_translate(k * 2.5, 0)(
            tr_homothety(0.2 + 0.2 * k)(centered)
        )
        for k in range(10)
    ]
    filtered1 = list(filter(flt_square(0.3), shapes1))

    axes[0, 0].set_title("6.1 До фильтрации (10 трапеций)")
    for poly in shapes1:
        xs, ys = zip(*poly)
        axes[0, 0].plot(xs + (xs[0],), ys + (ys[0],), color='gray')
    axes[0, 0].axis("equal"); axes[0, 0].axis("off")

    axes[0, 1].set_title(f"6.1 После фильтрации ({len(filtered1)} трапеций)")
    for poly in filtered1:
        xs, ys = zip(*poly)
        axes[0, 1].plot(xs + (xs[0],), ys + (ys[0],), color='blue')
    axes[0, 1].axis("equal"); axes[0, 1].axis("off")
